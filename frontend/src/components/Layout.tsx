...
import { Bookmark, User, Heart, Sparkles, LogIn, LogOut } from 'lucide-react';
...
export default function Layout() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-screen bg-[#f0f2f5] text-slate-800 font-sans overflow-hidden select-none">
      <AuthModal />

      {/* ========================================================= */}
      {/* LEFT SIDEBAR: Classic Tinder Desktop Navigation Panel      */}
      {/* ========================================================= */}
      <aside className="w-full md:w-[350px] lg:w-[380px] bg-white border-r border-slate-200/90 flex flex-col flex-none z-30 shadow-xs h-auto md:h-full">
        {/* Top Profile Header: Gradient Pink/Coral Bar */}
        <div className="flex-none h-16 bg-gradient-to-r from-[#fe3c72] via-[#ff655b] to-[#ff7854] px-4 py-2.5 flex items-center justify-between shadow-sm">
          ...
          >
            <div className="relative">
              <div className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-md border-2 border-white flex items-center justify-center font-bold text-sm text-white shadow-xs">
                {initials}
              </div>
        ...
        {/* Tinder "Discover Memes" Hero Card (matches screenshot's "Discover New Matches") */}
        <div className="p-3.5 border-b border-slate-100">
          <button
            ...
            className={clsx(
              ...
              isSwipingRoute
                ? "bg-gradient-to-r from-rose-50 to-orange-50 border-2 border-[#fe3c72]/30 shadow-xs"
                : "bg-slate-50 hover:bg-slate-100/80 border border-slate-200"
            )}
          >
            <div className={clsx(
              "w-12 h-12 rounded-2xl flex items-center justify-center text-xl transition-transform group-hover:scale-105 shadow-xs flex-none",
              isSwipingRoute
        ...
        {/* Sidebar Content List: Real-time Matches & Chats (matching Tinder screenshot messages list) */}
              ...
              return (
                ...
                >
                  <div className="relative flex-none">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm shadow-xs group-hover:scale-105 transition-transform">
                      {matchInitials}
                    </div>
                    <span className="absolute -bottom-1 -right-1 px-1.5 py-0.2 rounded-full bg-[#10b981] text-white text-[9px] font-bold border-2 border-white shadow-xs">
                      {percent}%
                    </span>
                  </div>
      ...
      {/* ========================================================= */}
      {/* RIGHT MAIN STAGE: Meme Card Deck or Full Page View         */}
      {/* ========================================================= */}
      <main className="flex-1 h-full relative overflow-y-auto flex flex-col bg-[#f0f2f5]">
        <Outlet />
      </main>
    </div>
  );
